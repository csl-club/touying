...
// Using icu-datetime package for lang's dates
#import "@preview/icu-datetime:0.2.2" as icu

#let display-nice-date(self) = {
  assert("info" in self, message: "self must have an info field")
  if type(self.info.date) == datetime {
    context {
        icu.fmt(self.info.date, locale: text.lang, date-fields: "YMDE", length: "long")
    }
  } else {
    self.info.date
  }
          ...
          grid(
            ...
            row-gutter: 0.7em,
            {
              ...
              if info.institution != none {
                title_extra = " @ " + info.institution
              }
              text(size: 1.3em, weight: "bold", info.organization + title_extra)
            },
            {
              if info.description != none {
                text(size: 0.9em, weight: "medium", info.description)
              }
            },
            {
              linebreak()
              if info.title != none {
                text(size: 1.2em, weight: "semibold", info.title)
              }
            },
          ),
          align(right + horizon, text(2em, utils.call-or-display(self, info.logo)))
        )
        line(length: 100%, stroke: .05em + self.colors.neutral-lightest)
        if extra != none {
          block(spacing: 1em, extra)
        }
        set text(size: .8em)
        if info.date != none {
          block(spacing: 1em, display-nice-date(self))
        }
        set text(size: .8em)
